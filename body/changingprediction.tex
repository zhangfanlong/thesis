...
%在此步骤中，当现有克隆组发生更改时，软件维护人员可以使用我们的预测变量来预测克隆一致性。如图~\ref{}~所示，我们的方法可以集成到一个IDE，如eclipse，可以保持克隆在开发阶段的变化。运行预测变量需要跟踪克隆在软件进化及其变化中的支持。一旦检测到克隆更改，该工具将提取与此更改实例相关的所有属性，预测其一致性要求，并通知开发人员。根据预测，开发者因此可以根据需要一致地保持克隆。
...
为评估本章方法，本章选取了四个开源软件进行实验。四个实验系统的克隆变化实例统计情况如表~\ref{changingsta}~所示。具体来说，第3列和第4列分别给出了不需要一致性维护和需要一致性维护的克隆变化实例的数量和比例。不需要一致性维护的克隆变化实例，该克隆变化不会使其所在的克隆组在未来演化中发生一致性变化，因此程序人员无需关注其克隆组的一致性问题。需要一致性维护的克隆变化实例，该克隆变化可能使其所在克隆组在未来演化过程中发生一致性变化，而遗忘这种变化会导致克隆缺陷，从而降低软件质量。
...
~&{不需要} &{需要} &  ~\\
~&{一致性维护}&{一致性维护}&~ \\
%~\multirow{3}{*}{\textbf{Project}}& \multicolumn{2}{c}{\textbf{Number (Percentage) of Changing Instances}} & \multirow{3}{*}{\textbf{Total}}\\ 
% \cline{2-3}
...
因为有两种类型的克隆变化实例，即需要一致性维护实例和不需要一致性维护实例。分别对上述两种克隆变化实例进行预测，可将实验划分为“一致性维护需求”实验（需要一致性维护）和“一致性维护自由”实验（不需要一致性维护）。同时，本章使用贝叶斯网络模型预测克隆创建实例的一致性，预测时贝叶斯网络会计算克隆变化实例的概率值，表示该实例需要一致性维护需求的概率(在0~1之间，数值接近于1表示需要一致性维护，接近于0表示不需要一致性维护)。
...
所创建的模型在预测{\em jFreeChart}和{\em Tuxguitar}的一致性需求时，预测结果显示非常有效，其中预测的准确度和召回率在80\%左右。同时，{\em jEdit}系统的预测效果虽然不如上面两个系统好，但是也相当不错。原因可能是因为jEdit系统的克隆变化实例规模较小，没有对模型实现较好的预测，但这需要进一步研究。尽管{\em  ArgoUML}的预测效果不如其它系统的预测效果，但其准确率仍然可以达到65\%左右，且召回率在45\%左右。最后，对于四个实验系统，本文所构建的模型均具有有十分合理的警告率，警告率十分接近于满足一致性维护需求的克隆变化实例的比例（如表~\ref {changsta}~中所示）。

%The models created have been very effective in predicting consistency-requirement satisfaction for both the projects {\em jFreeChart} and {\em Tuxguitar}, with both the precision and the recall of the prediction hovering around 80\%.  It performs fairly well, though not as good, for {\em jEdit} repository. This might be due to the small size of the repository, but further investigation is required. Although {\em ArgoUML} do not have a good prediction like the other repositories, it still can predict the requirement with precision around 65\% and recall around 45\%. For all the projects, our models have produced fairly reasonable warning rates; it is around to the percentage of change instances meeting consistency requirements (as listed in Table~\ref{t_1}).
...
%While variation in threshold does affect both the precisions and the recalls, it has stronger impact on the recalls than the precisions. This means that, the models created are pretty stable in providing, but can be further improved on its ability to recall. This implies that developers can quite confidently rely on the warning given by the predictors.
...
%To quantitatively assess the impact of each attributes, we conducted a one-tail t-test for the significance of difference between two means of correlated pairs, with the null hypothesis set as {\em the mean of precision/recall of all-attribute sample is the same as that of the sample without one set of attributes}.  The details are shown in the Appendix. The t-tail results reveal that there is a significant decrease in the recall-ability of consistency-requirement when either code attributes or context attributes were absent. However, the differences for the case of absence of evolution is not as significant as desired. Further investigation shows that evolution attributes do not aid significantly in the recall-ability in the {\em ArgoUML} repository. However, these attributes do contribute significantly to the other repositories. Further investigation is required to discover the true reasoning behind this discrepancy.  
...
%In addition, we employed the attribute selection function to obtain a best subset of attributes which can be used to perform prediction. The subsets obtained varies according to different experiment data (ie., different software repositories). We then compared these results with those obtained by using full attribute sets. The results consistently show that all-attribute can predict better than subset-attributes. While some of the attributes might not have significant contribution to the experiment results, none has adverse impact either. Nevertheless, it's important to keep these attributes as their contribution might become significant for other repositories. 
...
%\begin{tabular}{cccccccccccccc}
%\toprule[1.5pt]
%\multirow{2}{*}{实验系统}&\multirow{2}{*}{阈值}&\multicolumn{3}{c}{\textbf{全部属性(\%)}}&\multicolumn{3}{c}{无代码属性(\%)}&\multicolumn{3}{c}{无上下文属性(\%)}&\multicolumn{3}{c}{无演化属性(\%)}\\
%\cline{3-14}
%&&{警告率}&{准确率}&{召回率}&{警告率}&{准确率}&{召回率}&{警告率}&{准确率}&{召回率}&{警告率}&{准确率}&{召回率}\\
%%\multirow{2}{*}{\textbf{Project}}&\multirow{2}{*}{\textbf{Threshold}}&\multicolumn{3}{c}{\textbf{All(\%)}}&\multicolumn{3}{c}{\textbf{Without Code(\%)}}&\multicolumn{3}{c}{\textbf{Without Context(\%)}}&\multicolumn{3}{c}{\textbf{Without Evolution(\%)}}\\
%%\cline{3-14}
...
实验结果如表~\ref{changecrossmeeting}~所示。从表中可以看出，与全属性实验相比，交叉验证实验的预测效果也大大下降，准确率和召回率都下降。因此，本文建议使用本文的预测模型预测自身系统的克隆变化实例。如果在软件开发的初始阶段，项目可能没有足够的克隆变化实例，本文建议使用第三章提出的方法在创建克隆代码时检测克隆代码的一致性问题。当随着软件演化以后，系统自身的变化实例将会变得更多，此时可以使用自身数据训练模型并预测克隆代码的一致性。

%From Table~\ref{t_6}, we notice that, all precisions and recalls suffer when prediction models are built and used across different projects. Combining this result and that from the first experiment, we can safely recommend that our model is effective in its prediction for repository that have already evolved for quite some time, where there are sufficient change instances.

%At the initial stage of software development, project may not have enough clone change instances from its software repository to train its own prediction well. How then could we perform prediction for such a new software development?

%Our suggestion is as follows: At the start of software development, we require developers to be vigilant in checking for consistency-requirement whenever clone groups are created. Cross-project models can be used to assist at this point with caution, or the model built by Wang et al. \cite{Wang2014} can be used when copy-and-paste operations are performed. Later, as software evolves through several versions, it's advisable that we retrain the model by progressively reducing the amount of cross-project data and increasing the amount of data from its own repository.

\begin{table}[htbp]
\bicaption[changecrossmeeting]{}{四个实验系统上项目交叉实验效果}
...
%The models built for projects {\em ArgoUML} and {\em jFreeChart} respectively have very effective prediction of consistency-free, with the precision hovering around 80\%, and  recall around 70\%. It performs fairly well, though not as good, for {\em jEdit} repository.  Unfortunately, our model can not predict good enough for {\em Tuxguitar}.This might be due to the small size of the change instance set (being consistency-free), but further investigation is required. For all the projects, our models have produced fairly reasonable recommendation rates.
...
%%While variation in threshold does affect both the precisions and the recalls, it has stronger impact on the recalls than the precisions. 
%%This means that, the models created are pretty stable in providing, but can be further improved on its ability to recall. 
%%This implies that developers can quite confidently rely on the warning given by the predictors, but may need other means to help in further identify those cases where the predictors remain silent. 
%In short, our approach produces models that offer a stable prediction with reasonably well precisions and recalls, bringing confidence to software developers about replying on the inference produced by the models. 
...
同样，对不需要一致性维护的克隆变化实例，本文同样进行了属性组实验，以评估不同的属性组对预测效果的影响。实验结果如表~\ref{changesetfree}~所示，其中，分别给出了使用全部属性和删除每一组属性后的实验结果，即“全部属性”、“无代码属性”、“无上下文属性”和“无演化属性”。
...
%Similar with the case of consistency-requirement prediction, we also performed one-tail t-tests. The details are also shown in section appendices. According to the results, we have high level of confidence that none of the attribute sets whose absence will have significant adverse impact -- in comparison with others -- on the precision. On the other hand,  the absence of {\em any one} of these attribute sets will {\em have significant adverse impact} on the recall values of the models trained. 
...
%We also build predictors by choosing the best subsets of attributes, obtained from WEKA's attribute selection function, aiming at exploring the effect of each attributes. Comparing with full attribute sets, the results consistently show that {\em all-attributes predictor has better predictive power than subset-attribute predictors.\/} 

尽管一些属性组可能对实验结果没有极为重要的贡献，但是属性组同样也没有产生消极的影响。因此，本文建议将所有属性保留在构建预测模型中，因为某些属性可能对一些未经实验验证的系统具有重要且积极的影响。

%Although some of the attributes might not have significant contribution to the experiment results, none of them has adversary impact either. Consequently, we recommend keeping all the  attributes in constructing prediction model, as some attributes might turn out to have significant contributions for some yet-to-be-explored repositories.
...
%\begin{table}[htbp]
%\bicaption[changesetfree]{}{在四个实验系统上属性组实验结果}
%{Table$\!$}{The effectiveness for attribute set on four projects}
%\vspace{0.5em}
%\centering
%\wuhao
...
在这个实验中，我们探讨使用跨项目在预测不需要一致性维护的克隆变化实例。同样，使用其它软件系统的克隆变化实例作为训练集，并使用另外系统作为测试集。实验结果如表~\ref{changecrossfree}~所示。

%%In this experiment, we explore the effectiveness of using cross-project in predicting consistency-requirement-free. we determine if models trained using data from other software repositories can be used to predict consistency-requirement satisfaction for a different software repository. With four subject repositories, in this experiment, we take turns to use data from three repositories to train a prediction model, and use the model to predict consistency-requirement-free of a clone change in the remaining repositories. 

从表中可以看出，所有的准确率和召回率都受到了严重的影响。结合全属性实验的实验结果（表~\ref{changeallfree}~），可以发现当使用自身系统的数据作为训练集且训练集足够大时，本文的预测模型有效的，例如软件系统经过长时间的演化具有了大量的克隆变化实例。因此，本文建议使用系统自身的数据进行一致性预测。然而，在软件开发的初期阶段，由于缺乏足够数量的克隆变化实例，不能很好的训练模型时，开发人员可以使用本文第三章提出的方法在克隆创建时预测克隆代码的一致性，从而仅允许那些不需要一致性维护的克隆实例产生。在软件演化到了足够的版本时，随着来自其自己的克隆变化数据的增加，开发人员可以使用自身数据重新训练模型以预测项目中克隆变化的一致性。

%%table~\ref{t_9} shows that all precisions and recalls suffer comparing with the effective experiment. Combining this result and the (first) Effectiveness Experiment, we observe that prediction model is effective when it is trained from its own repository having sufficient number of change instances; ie., when the repository has already evolved for quite some time. Therefore, we recommend that, at the infancy stage of software development, due to a lack of sufficient number of clone change instances for training the model well, the developer can use the model bulit by Wang {\sl et al.} \cite{Wang2014} that detects copy-and-paste operations to avoid clones. After several versions of evolution of the software, and with increasing amount of data from its own repository, the developer can retrain the model to predict consistency-free of clone changes in the project.
...
本章从不同的角度评估了所建立的贝叶斯网络模型的预测能力，同时对不需要一致性维护和需要一致性维护的克隆变化实例进行了一致性预测。实验结果表明，当系统使用自身的克隆变化实例进行全属性预测时，本章的模型在一致性维护需求和一致性维护自由的实验上具有有效的预测能力。而通过属性组实验发现每一个属性组在不同角度和程度上对预测模型的效果有着积极的作用。因此，本文鼓励开发人员使用全属性组进行克隆变化实例的一致性维护需求预测，从而将其适用到其它系统是达到最佳的预测效果。

%We evaluated the prediction power of Bayesian network model built to predict both on consistency-requirement and free for all the clone change instances. Based on the results of our experiments, our models have effective prediction ability, on both consistency-requirement and consistency-free prediction, when they are trained with full attributes for all instances.  The experiments on omitting single attribute set reveal each of the attribute sets contribute significantly -- to various degree -- to the quality of the prediction model. Therefore, developers should be encouraged to add in other attributes along these three dimensions to further sharpen the predictability of the constructed model. 
...
%The cross-project experiment shows that prediction model effectiveness is highly dependent of the specific characteristics of the repository on which the model is to be applied. This implies that it may be ineffective to build a "universal" predictive model that can predict consistency-requirement/-free of clone changes for all repositories. In other words, developers will need to create different models for different repositories. This is certainly not entirely desirable, and we feel that further investigation is required here.
...
%The ineffectiveness of predictive power of model coming from cross-project repositories also implies that the current approach has a limitation in bootstrapping itself during model building, as there are not sufficient change instances at the infancy stage of software development to support the construction of prediction model. Here, we recommend that developers to use the model developed by Wang {\sl et.al.} at the early stage of software development, and use our model as the software has evolved for some versions.
...
%The presence of clones in software adds to the burden of software maintenance due to the potential need to maintain consistent change in clones when software evolves.  In this paper, we propose an approach to predict clone consistency-requirement for any clone group which has some of its constituent clone fragments undergoing code modification. We build clone genealogies from the software repositories to extract clone change instances. For each change instance, we extract three sets of attributes.  Specifically, we introduce a set of {\em evolution attributes} to capture the characteristics of evolution history. Consequently, our attributes {\em provide a holistic view about code change, from perspectives of individual, environment, and evolution.  We use these data to build a Bayesian network for each repository, implemented using WEKA.  We conduct experiments on four open source software projects to investigate three perspectives of the prediction models.  The experimental results show that the prediction models have reasonable rate: it has good precision and recall rates for two larger piece of repositories, and reasonable precision and recall rates for the smallest repository.In addition, all three sets of attributes have strong impact on recall rates of the prediction models. Our experiment indicates that cross-project prediction may not be a good choice for quality prediction.

%The main cost of predictor construction arises from change instance collection from repositories; this cost can easily be amortized over software evolution, making this approach scalable. To follow up, we intend to enhance the recall-ability of the predictor by introducing new attributes or attributes selection.We also intend to build a general cross-project predictor to enhance the ability for new software repository which do not have enough data to train its model well. With this cross-project predictor, developers can ignore the training steps at the developing, which can really save the time of clone consistency prediction.We believe that this holistic approach can be a solid boost to software maintainability to improve software quality, as developers can now be more aware of when consistency-requirement of a clone group need to be investigated, avoiding the potential risk of consistency-defects. 