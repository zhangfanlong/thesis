...
%因此，本章与第3章相似，在克隆代码变化时（克隆代码修改时）使用机器学习方法预测其一致性维护需求。但是，与第3章不同的是，本章定义的克隆代码变化时的一致性变化，不仅要求克隆代码同时被修改，还要求发生相同的变化。
%首先，通过构建软件系统的克隆家系，并识别来收集系统中所有的克隆变化实例（发生变化的克隆组）。然后，提取了代码属性、上下文属性描述发生变化的克隆组，并新增一组演化属性描述其历史演化情况。最后，训练机器学习模型预测克隆代码变化的一致性维护需求。
...
当克隆组内的克隆片段的变化需要保持一致，而开发人员遗忘对其它的克隆片段进行一致性变化时，可能会向系统中引入克隆一致性违背缺陷（Consistency Defect）\cite{bettenburg2009empirical} \cite {juergens2009code}。先给出一个可能导致缺陷的克隆代码一致性变化示例，如图~\ref{changingexamplea}~和~\ref{changingexampleb}~所示。该一致性变化例子来自于软件系统jEdit中。在图~\ref{changingexamplea}~中给出了$ 3.1.0 $版本中的两个克隆代码片段。在随着系统演化至$ 3.2.0 $版本时，上述两个克隆代码发生了一致性变化。具体地，$ 3.1.0 $版本中的第9-12行和第15行代码在下一版本$ 3.2.0 $中被开发人员修改。第9-12行，开发人员将{\tt selectionEnd}更改为$ 0 $，并删除了一对花括号。在第15行，开发人员删除了声明语句。如图~\ref{changingexampleb}~所示，这两个被修改的代码片段仍然是彼此相似的克隆代码（仍存在于版本$ 3.2.0 $中的同一克隆组中）。假设开发人员没有对上述两个克隆代码进行一致性维护，这必然会导致克隆代码的一致性违背缺陷，从而降低软件质量。
...
当克隆组内的克隆代码发生变化时，需要判断此变化是否会导致克隆代码的一致性变化，而判断一致性变化也会导致额外的维护代价。当克隆组需要一致性变化时，遗忘一致性变化会引发一致性违背缺陷。克隆代码的变化问题会增加软件的维护代价，且可能引发缺陷使得软件质量下降。

\BiSubsection{现有研究存在的问题}
{Problems in Current Research}

研究表明，研究发现在克隆代码的演化过程中，会存在相当数量的克隆代码发生一致性变化\cite{saha2011automatic}。与此同时，本文第2章第~\ref{ref-characteristics}~节挖掘了克隆代码的演化特征，也发现软件中存在相当数量的克隆代码一致性变化。然而，如图~\ref{changingexamplea}~和~\ref{changingexampleb}~所示，克隆代码的一致性变化很可能会引发克隆代码的一致性违背缺陷。
...
为了维护演化中的克隆代码，尤其是发生变化的克隆代码，研究人员展开了对克隆代码变化维护的研究，主要包括两个方面克隆变化预测研究和克隆一致性维护研究。
在克隆代码的变化预测研究中，Mondal等人通过对克隆组内的克隆代码排序，从而预测需要一致性维护的克隆代码\cite{mondal2014prediction}。而Murakami等人在克隆代码未发生变化时，预测克隆代码是否会发生下一次变化\cite{murakami2014predicting}。上述研究没有在克隆代码变化时进行预测，而是在较早的时间对其提前预测，仅仅是帮助开发人员提高对可能发生变化的克隆代码保持警惕性。
对克隆代码的一致性维护研究中，Yamanaka等人\cite{yamanaka2013applying}将克隆变化与事件通知机制相结合，将每一个克隆变化都看成一个事件，在克隆发生变化时提醒开发人员及时地对克隆变化进行维护。但是，该方法会将所有的克隆代码变化通知开发人员，没有对是否需要一致性维护进行判定，仍需开发人员手工验证。Cheng等人\cite{cheng2016rule}提出了一个基于Token的克隆代码一致性维护方法，当克隆组内的一个代码发生变化时，能够同时修改组内的其他克隆代码，保证克隆代码的一致性。同时，克隆管理工具JSync\cite{nguyen2012clone}中，也支持克隆代码的重命名一致性维护。但是，在上述方法中，研究人员假设该变化需要一致性维护，缺少对克隆变化是否需要一致性维护的判定。

现有对克隆代码变化维护研究依然存在以下两点不足之处：

（1）在目前的克隆变化预测研究中，通过分析历史版本中的克隆代码的变化情况，在克隆代码未发生变化时，提醒开发人员对存在可能变化的克隆代码。但是，克隆代码的变化以及一致性维护，是一个实时动态的过程。未在克隆代码发生变化时，进行预测难以将其与软件开发过程相结合，也无法在实际开发过程中，提示程序开发人员对变化的克隆代码进行一致性维护。

（2）在目前的克隆一致性维护研究中，研究人员假定克隆代码的变化一定会需要一致性维护，并支持了这种一致性维护操作。但是，实证研究表明并不是全部的克隆变化都需要一致性维护，其中有接近一半的克隆代码的变化不需要维护\cite{krinke2007study}。因此，当前的研究更缺少对发生变化的克隆代码是否需要一致性维护的判定研究。
...
与第3章的定义不同，克隆变化时的一致性变化包含两个约束条件。第一个约束条件要求 $CF_1 $和$CF_2 $同时被修改，第二个约束条要求$CF_1 $和$CF_2$发生相同的变化。在第二个约束条件中，一致性变化使用一个非常小的阈值$\tau$描述，要求克隆片段发生了完全一致的修改（本章中$\tau$取值为$0.003$）。
