...
为评估本章方法，本章采用和第3章相同的四个Java开源软件进行实验（如表~\ref{statisticsprojects}~所示）。本章通过遍历克隆家系中发生变化的克隆组（一致性和不一致性变化模式）来收集克隆变化实例。本章所采用的实验系统是经过充分演化的开源系统，并且在克隆代码相关研究中已经被大量采用。同时，所选择的软件版本均为正式的发布版本，系统所有的版本经专业的开发团队测试后发布。因此，本章所收集的克隆变化是值得信赖的正确的代码变化。

四个实验系统的克隆变化实例统计情况如表~\ref{changesta}~所示，第2列和第3列分别给出了不需要一致性维护和需要一致性维护的克隆变化实例的数量和比例。对于不需要一致性维护的克隆变化实例，该克隆变化不会使其所在的克隆组在未来演化中发生一致性变化，因此程序人员无需关注其克隆组的一致性问题。对于需要一致性维护的克隆变化实例，该克隆变化可能使其所在克隆组在未来演化过程中发生一致性变化，而遗忘这种变化会导致克隆缺陷，从而降低软件质量。
