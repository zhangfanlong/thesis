...
综上，目前的研究中缺少在克隆代码变化时的一致性变化的预测研究，即当克隆组内某一克隆代码发生变化时，预测该变化是否会导致一致性变化。
...
该定义包含两个约束条件：第一个约束条件要求 $CF_1 $和$CF_2 $同时被修改，第二个约束条要求$CF_1 $和$CF_2$发生相同的变化；在第二个约束条件中，一致性变化使用一个接近于0的阈值$\tau$描述，要求克隆片段发生了完全一致的修改。%本章中$\tau$取值为$0.003$，原因是本章使用。
