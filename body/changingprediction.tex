...
为全面评估本文方法，将实验进一步的划分为全属性实验、属性组实验两：
...
本节使用使用开源软件工具WEKA训练和预测贝叶斯网络。在构建贝叶斯网络时，使用K2搜索算法\cite{}建立网络结构，并设置贝叶斯网络最大父节点个数为4，使用{\em  SimpleEstimator}来估计贝叶斯网络的条件概率表。在实验时，采用十倍交叉验证(10-folds Cross-Validity)的方式，将克隆变化实例的数据集划分为训练集和测试集，使用训练集训练贝叶斯网络生成预测模型，然后使用测试集评估贝叶斯网络的预测能力。
