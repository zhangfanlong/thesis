...
构建克隆家系的算法如~\ref{alg-building}~所示。算法中第1-4行为使用NiCad检测克隆代码过程。第5-8行为克隆代码生成CRD。第9-12行为使用CRD映射相邻版本的克隆代码，并根据克隆演化模式定义，识别克隆演化模式。第14行生成系统中全部的克隆家系。时间复杂度为。
...
\label{alg-building}
\KwIn{All source code from $N$ versions}
...
克隆演化特征的聚类算法如~\ref{alg-characteristic}~所示。算法的第1-3行分别收集系统中全部的克隆实体。第4行初始化克隆实例的聚类空间。第5-8行、9-12行、13-16行分别使用上文描述的度量生成克隆聚类向量，并将其添加到聚类向量空间中。最后，第17行调用WEKA聚类克隆实体。算法复杂度为
...
\KwIn{All clone genealogy $CGEs$ and all source code}
\KwOut{All the clusters for clone fragments, groups, and clone genealogies}
Collecting all the entities of clone genealogy with number {$N\_cge$}\;
Collecting all the entities of clone group with number{$N\_cg$}\;
Collecting all the entities of clone fragment with number {$N\_cf$}\; 
