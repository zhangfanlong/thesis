...
\begin{definition}[一致性变化（Consistent Change）]  
%\label{def-cochange}
假设存在两个克隆代码片段 $CF_1$和 $CF_2$，且它们被分别地修改为$CF'_1$和$CF'_2$。 给定一个非常小的阈值$\tau$，如果克隆代码$CF_1$和$CF_2$的变化满足以下条件称此变化为{一致性变化（Consistent Change）} , 
  \[
  \begin{array}[t]{crl}
...
\begin{definition}[一致性变化模式（ Consistent Change Pattern）] 
...
\begin{definition}[克隆实例（ Clone Instance）] 
\label{def-instance}
...
\begin{definition}[克隆一致性维护需求（Clone Consistency-Requirement）] 
