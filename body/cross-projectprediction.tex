...
与第四章类似，首先基于贝叶斯网络的机器学习模型，对克隆代码的一致性需求预测问题进行实验，用于观察跨项目能否预测克隆代码的一致性维护需求预测。
然后，在五种不同的机器学习方法进行跨项目的克隆代码一致性需求预测。
系统所选择的评估指标同第三章和第四章的相同。
...
\BiSubsection{基于贝叶斯网络跨项目一致性预测}
{Experiments for Clone Consistency-Requirement}

\BiSubsection{克隆代码创建时跨项目预测}
{}
...
因此，跨项目预测实验中，结果表明本章所提出的预测模型的有效性会依赖于系统自身的某些特征。这意味着使用本章所提取的属性构建一个“通用”预测模型时，可能较为困难的预测某一特定系统的一致性维护需求。
...
\BiSubsubsection{多对一跨项目一致性预测实验}
{Clone Consistency Prediction Experiment}

本节还在其它机器学习方法上

\begin{table}[htbp]
\bicaption[changecrossfree]{}{克隆代码创建时平均多源机器学习交叉实验效果}
{Table$\!$}{The effectiveness for cross-project on four projects}
\vspace{0.5em}
\centering
\wuhao
...
\begin{table}[htbp]
\bicaption[changecrossfree]{}{克隆代码修改时平均多源机器学习交叉实验效果}
{Table$\!$}{The effectiveness for cross-project on four projects}
\vspace{0.5em}
\centering
\wuhao
